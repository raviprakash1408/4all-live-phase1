...
const LivekitTileElement = ({
  ...
  useEffect(() => {
    try {
      if (type !== "device") {
        if (video && !videoMuted) {
          video.attach(document.getElementById(`video-${trackUniqueId}`));
        } else {
          video?.detach();
        }

        setuserShortName(shortNameCreator(user.firstName, user.lastName));
      }
    } catch (error) {
      console.log(error);
    }
  }, [trackUniqueId, type, user.firstName, user.lastName, video, videoMuted]);
    ...
    <>
      <div
        ...
        style={{
          ...
          cursor: "pointer",
          position: "relative",
          rotate: window.mobileCheck() && conference.layout == "stage_layout" ? '90deg': '0deg',
          outline: hover
            ? "3px solid #0E558C"
        ...
        onClick={(e) => {
          if (
            ...
            window.eventChanel.publish({
              ...
              trackUniqueId,
              // userId,
              // clickedby: window.room?.myUserId(),
            });
            goToCenterStage(e);
...
export default LivekitTileElement;
